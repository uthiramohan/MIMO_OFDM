...
for i = 1:length(snr)
    for j = 1:c                                         % Transmit frame by frame
        hx = filter(h,Tx_Data(:,j).');                  % Pass through rician channel
        a = h.PathGains;
...
title('SNR vs BER for BPSK/QPSK/8-PSK/DBPSK/DQPSK/8-DPSK/16,64,256-QAM MIMO OFDM over Rician Fading Channel');
xlabel('EbNo');
ylabel('BER');